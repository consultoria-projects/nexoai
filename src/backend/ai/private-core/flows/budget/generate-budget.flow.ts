...
import { validationAgent } from '@/backend/ai/private-core/agents/validation.agent';
        ...
        for (let i = 0; i < subtasks.length; i++) {
            ...
            try {
                ...
                if (resolvedItem) {
                    ...
                    if (ctx?.userId) {
                        const { emitGenerationEvent } = await import('@/backend/budget/events/budget-generation.emitter');
                        const type = ('sku' in resolvedItem || (resolvedItem as any).type === 'MATERIAL') ? 'MATERIAL' : 'PARTIDA';
                        emitGenerationEvent(ctx.userId, 'item_resolved', { item: resolvedItem, type });
                    }
