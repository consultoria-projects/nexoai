...
import { subtaskExtractionPrompt } from '@/backend/ai/private-core/flows/budget/extraction.flow'; // Inline prompt (filesystem-independent)
...
export const constructionAnalystAgent = ai.defineFlow(
    ...
    async (input) => {
        ...
        if (description) {
            ...
            // Generate using the inline prompt (filesystem-independent for production/serverless)
