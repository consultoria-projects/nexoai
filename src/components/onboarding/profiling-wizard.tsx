...
export function ProfilingWizard({ leadId, onComplete, intendedAction }: ProfilingWizardProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (!data.biggestPain?.length || !data.simultaneousProjects || !data.role ||
            !data.companyName || !data.companySize || !data.currentStack?.length || (!noWeb && !data.web)) {
            setError('Por favor, rellena todos los campos o marca "No tengo web por ahora" si no dispones de página web.');
            return;
        }
    ...
    return (
        <div className="w-full max-w-2xl mx-auto flex flex-col h-full justify-start overflow-hidden pt-2">
            {/* Header & Progress */}
            <div className="mb-8">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-sm font-medium text-muted-foreground">{p.common.stepIndicator} {step} {p.common.of} 3</div>
                    <div className="w-9" />
                </div>
            ...
            {/* Content Area */}
            <div className="relative flex-1 flex flex-col w-full justify-start overflow-y-auto custom-scrollbar pb-8 pr-2 min-h-0">
                            ...
                            <div className="w-full max-w-2xl space-y-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-3">
                                        <label className="text-sm font-semibold">{p.step4.webLabel || 'Página Web'} <span className="text-red-500">*</span></label>
                                        <input
                                            type="url"
                                            value={data.web || ''}
                                            ...
                                            className={`w-full px-4 py-3 bg-secondary/30 border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-opacity ${noWeb ? 'opacity-30 cursor-not-allowed hidden' : ''}`}
                                        />
                                        <div className="flex">
                                            <button
                                                type="button"
                                                onClick={() => {
                                                    const newNoWeb = !noWeb;
                                                    setNoWeb(newNoWeb);
                                                    if (newNoWeb) handleSelect('web', '');
                                                }}
                                                className={`px-4 py-2 rounded-full text-xs font-semibold transition-colors border ${noWeb ? 'bg-primary/10 text-primary border-primary/20' : 'bg-secondary/30 text-muted-foreground border-border hover:bg-secondary/50'}`}
                                            >
                                                {noWeb ? 'No tengo página web (Omitido)' : 'No tengo página web'}
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div className="flex justify-end pt-4">
                                    <button
                                        ...
                                        disabled={submitting || !data.companyName || !data.companySize || !data.currentStack?.length || (!noWeb && !data.web)}
